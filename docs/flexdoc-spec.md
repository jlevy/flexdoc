...
- `Link(text, url, title, span, form)`: identity from `flowmark.markdown_ast.extract_links`
...
- **`links()` returns navigable links only by default** (`TRUE_LINK_FORMS`: `inline`,
  `reference`, `autolink`, `bare_url`); `links(forms=…)` selects any forms and `images()` is
  ...
  `collect()` finds them — and via `links(forms={LinkForm.reference_definition})`, never the
