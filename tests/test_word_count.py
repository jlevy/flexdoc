...
def test_text_units_distinguish_raw_and_logical_plaintext_words():
    text = "<p>你好世界</p>"
    assert {unit.value for unit in TextUnit} >= {"raw_words", "words"}
    assert "logical_words" not in {unit.value for unit in TextUnit}
    assert size(text, TextUnit.raw_words) == 1
    assert size(text, TextUnit.words) == 2


def test_document_logical_words_round_once_across_all_paragraphs():
    doc = FlexDoc.from_text("a.\n\nb.\n\nc.")
    assert doc.size(TextUnit.words) == logical_word_count(doc.reassemble()) == 2
    assert sum(paragraph.size(TextUnit.words) for paragraph in doc.paragraphs) == 3
...
def test_section_sizes_and_tree_default_to_logical_words():
    ...
    assert section.size(TextUnit.words) == 5
    assert doc.section_size_tree() == "# T  (5 words)"
    assert "5 words" in doc.size_summary()
...
def test_seek_to_sent_supports_both_word_units():
    ...
    for unit in (TextUnit.raw_words, TextUnit.words):
